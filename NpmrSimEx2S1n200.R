##########################################################################################################
##########################################################################################################
####### Simulation for nonparametric mixture of regression (Scenario 1, n=200, EX2 independent covariates)
##########################################################################################################
##########################################################################################################
...
pi_1 <- function(x){
  return(exp(0.3*x[,1]+0.2*x[,2])/(1+exp(0.3*x[,1]+0.2*x[,2])))
}
...
m_1 <- function(x){
  return(4+15*dmvnorm(2*x-1,sigma = 0.3*diag(2)))
}
m_2 <- function(x){
...
X_pi <- X_sig2 <- X_m <- cbind(rep(1,n),x,x^2,x[,1]*x[,2])
...
bs_AMISE <- function(j){ 
  ...
  initl_beta_pi <- c(rmt$eps[1],0,0,0,0,0)
  initl_beta_sgm21 <- c(rmt$var[1],0,0,0,0,0)
  initl_beta_sgm22 <- c(rmt$var[2],0,0,0,0,0)
...
write.csv(c(h_AMISE,RASE_mean,RASE_sd),"ex2outputn200")
