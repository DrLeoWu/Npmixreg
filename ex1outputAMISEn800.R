...
##############################################################################################################################
############## Select bandwidth based on AMISE for 10 simulations (n=800)
set.seed(389)
n <- 800
...
sim_npmixreg <- function(i){
  ...
  library(interp)
  y <- rnpnormmix(p,mu,sgm)
  output <- npnormmixEM_grid(x,y,N=100,initl_m=initl_m,initl_sgm=initl_sgm,initl_pi=initl_pi,bw=h_AMISE) 
...
write.csv(c(h_AMISE,RASE_mean,RASE_sd),"ex1outputAMISEn800.csv")
